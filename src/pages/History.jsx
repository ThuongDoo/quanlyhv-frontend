  ...
  return (
    <div className="min-h-screen bg-slate-50 font-sans">
      ...
      <div className="max-w-3xl mx-auto px-6 py-8 flex flex-col gap-6">
        ...
        {/* Bảng */}
        {loading ? (
          <p className="text-sm text-slate-400 text-center py-10">Đang tải...</p>
        ) : history.length === 0 ? (
          <p className="text-sm text-slate-400 text-center py-10">Chưa có dữ liệu trong ngày này.</p>
        ) : (
          <div className="overflow-x-auto rounded-2xl border border-slate-200 bg-white shadow-sm">
            <table className="w-full text-sm text-slate-700">
              ...
              <tbody>
                {history.map((row) => {
                  const shiftLabel = SHIFTS.find((s) => s.value === row.shift)?.label ?? row.shift;
                  return (
                    <tr key={row._id ?? row.shift} className="border-t border-slate-100 hover:bg-slate-50 transition">
                      <td className="px-4 py-3 font-bold text-indigo-600">{shiftLabel}</td>
